...
- 当客户端在 1 秒后没收到服务端的 SYN-ACK 报文后，客户端就会重发 SYN 报文，重发的次数由 `tcp_syn_retries` 内核参数控制，默认值一般是 5
...
- 当第五次超时重传是 16 秒，当第五次超时重传后，会继续等待 32 秒，如果服务端仍然没有回应 ACK，客户端就不再发送 SYN 包了，然后断开 TCP 连接
...
客户端收到服务端的 SYN-ACK 报文后，就会给服务端返回一个 ACK 报文，也就是第三次握手，此时客户端进入 `ESTABLISHED` 状态
