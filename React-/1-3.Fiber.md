...
React16 之前的版本比对更新 VirtualDOM 的过程是采用循环加速递归实现的，这种比对方法有一个问题，就是一旦任务开始进行就无法中断

- 如果应用中逐渐数量庞大，主线程被长期占用，直到整颗 VirtualDOM 树比对更新完成之后主线程才能被释放，主线程才能执行其他任务
...
3. 任务拆分，将任务拆分成一个个小的任务
   ...
   - 以前我们将整个一个 virtualDOM 的比对看成一个任务，现在我们将树中每一个节点的比对看作一个任务，这样一个个大的任务就拆分成一个个小任务了
...
# 创建任务队列和完成任务调度逻辑"requestIdleCallback"
...
我们在 render 方法中调用 Fiber 算法核心 API `requestIdleCallback`，完成指定在浏览器空闲时执行任务，我们将任务函数取名为 performTask
...
2. 实现 performTask 方法
   ...
   Fiber 算法会将一个大任务拆分成一个一个小任务，一个个小任务就需要采用循环的方式来调用，所以 performTask 做的第一件事就是循环调用，我们将这个事件处理函数命名为 workLoop，并将 deadline 传递进去
...
3. 实现 workLoop 方法
   ...
   **当任务存在**
   ...
   如果任务存在并且浏览器有空余时间我们则需要执行这个任务，而且这个 subTask 任务不止一个，所以我们采用循环的方法去执行这个任务。我们再封装一个函数  executeTask 代表执行任务，executeTask 执行完以后必须返回一个新的任务回来，只有返回一个新的任务这个 while 循环才能继续去执行，executeTask 并且接收一个参数，实际上这个参数就是 fiber 对象
...
这时候就完成了每个 DOM 对象构建成 Fiber 的工作。这里还有点小问题：第一个子节点才是父节点的子节点，其他节点都是第一个子节点的兄弟节点。所以只有第一个子节点才去设置父节点的 child 属性，其他的都需要设置兄弟节点
...
以上我们已经把根节点、它的第一层子节点和第一层子节点的兄弟节点都构建了 fiber 对象并且关联了起来，接下来我们要继续往下查找子节点继续构建 fiber 对象

当执行完 reconcileChildren，如果 fiber.child 有值，executeTask 返回一个新的 fiber 对象，workLoop 方法里 subTask 重新被赋值继续执行 executeTask。直到 executeTask 子节点为空，结束循环
...
当左侧节点构建完成之后，我们定位的应该是最后一个子节点，就根据这个最后一个子节点去查找剩余节点。如果当前节点有同级就去构建该节点，如果没有就退回它的父级，查看它的父级有没有同级，一直退回查找构建 fiber 对象，将所有剩余子节点构建 fiber 对象
...
- 当左侧节点树种的节点全部构建完成以后，我们开启一个 while 循环去构建其他节点过程时，我们会找到每一个节点的**父级 fiber 对象**，这样我们就可以为每一个节点 effects 数组添加 fiber 对象了
...
首先我们准备好一个类组件，类组件需要继承 Component 类，并新建一个文件夹 Component 在里面创建 index.js 存放 Component 类
...
如果是组件，它的子级就不是直接 fiber.props.children 了，而是类组件里面 render 方法返回的内容，所以调用 reconcileChildren 方法的时候需要判断是组件还是普通元素，是组件的话调用 render 方法
...
因为类组件本身也是一个节点，但是类组件本身的节点是不可以追加元素的。所以我们要往父级查找，直到父级不是组件而是普通元素，才开始做元素追加操作
...
这两个 JSX 只是从 **Hello React** 替换成 **奥利给** 的操作。当 DOM 初始化完成渲染之后，我们备份旧的 fiber 对象，在两秒钟之后又调用了 render 方法，就会去创建新的 fiber 对象了
