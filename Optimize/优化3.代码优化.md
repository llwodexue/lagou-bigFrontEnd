...
当 Chrome 或 Node 要执行一段 JS 代码时，首先会进行解析（Parse），将源代码翻译成抽象语法树（AST）。解析器把文本识别成字符（Token），然后把重要信息提取出来，变成一些节点存储在树形数据结构里。接着解释器（Ignition）将 AST 编译为字节码（Bytecode）执行。在代码被频繁执行时，V8 的优化编译器（TurboFan）会将热点代码编译为高度优化的机器码。但有时运行时实际情况与编译时的假设不符，会发生去优化（Deopt），所以需要我们在代码层面尽量满足 V8 的优化条件，编写有利于优化的代码
...
- 对于 V8 引擎而言，实际上是有类型的
  ...
  JavaScript 不区分整数、浮点数，它们都是 number 类型，但是在 V8 引擎内部会对类型做出精确的区分（Smi 表示小整数，Double 表示浮点数）。如果数组里面类型发生变化，就会造成额外的开销，效率就不高了
...
- 元素类型只能向下过渡，一旦将浮点数添加到 Smi（小整数）数组中，即使稍后用 Smi 覆盖浮点数，它也会被标记为 DOUBLE。类似地，一旦在数组中创建了一个洞（hole），它将永久标记为 HOLEY，即使后来填充了该位置也是如此
