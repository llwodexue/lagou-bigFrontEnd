...
- 事件（`touch`、`wheel`、`pointermove` 等）开始触发，JS 触发视觉上的变化
- 一帧开始，rAF（`requestAnimationFrame`）回调执行，Layout（重新计算样式，更新图层树）布局，Paint（绘制）重绘，Composite（合并图层）合成
- rAF 是在布局和重绘之前调用，这样可以利用 rAF 先把我们做的处理做完，之后再去进行布局和绘制，极大提高效率
- rAF 本身是由浏览器进行调度的，会在每一次绘制之前去触发这个 rAF 回调，尽量达到 60fps 的效果
