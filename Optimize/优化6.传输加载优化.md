...
这个技术可以帮助我们对 TCP 连接进行复用，也就是说当我们和一台服务器进行 TCP 建立连接之后，接下来的请求就都不需要重复建立连接。Nginx 默认开启 `keep-alive`

- 它是 HTTP 标准中的一部分，多数情况是有益无害的，所以在 HTTP1.1 以后，Keep Alive 默认开启
- Initial connection 为 TCP 连接的建立，后续资源加载就没有 Initial connection
...
```nginx
http {
  keepalive_timeout  65; # 超时时间，65s内没使用TCP连接就会断掉
  keepalive_requests 100; # 客户端和服务端进行TCP连接后，会开始计数，第101个请求就需要重新建立 TCP连接
}
```
...
Service Worker 也有自己的生命周期，首先要注册、安装、激活才能使用。打包后的目录里会生成 `asset-manifest.json`，里面定义了哪些资源要进行缓存、缓存文件的文件名。相关的版本信息会存在 `precache-manifest` 里，每个文件都有相关版本信息
...
- 执行之后得到 `server.crt` 和 `server.key`，在工程目录下新建 ssl 文件夹，将其拷贝进去
