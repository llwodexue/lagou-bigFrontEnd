...
**URL 解析：**浏览器会判断输入的字符串是搜索的关键词还是访问站点的 URL
...
- 建立连接过程中还要看协议，如果是 HTTPS 需要建立 TLS 连接

- 如果碰到 301 （永久移动）则要重新发起请求

- Server 上处理请求，最后会组织成 Response 返回给前端

  读到 Response 前几个字节会开始分析数据的类型，虽然服务器会在 `Content-Type` 中声明这个类型是什么，但是这个不一定是正确的，浏览器会对此进行判断（安全检查）
