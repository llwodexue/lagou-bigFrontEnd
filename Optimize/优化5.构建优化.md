...
- `terser-webpack-plugin`：支持压缩 ES6 (Webpack4)
  ...
  terser 无论从性能还是效果上都比 uglifyjs 好，所以 terser 后面作为生产模式下默认的压缩插件
...
- splitChunks 提取公有代码，拆分业务代码与第三方库
  ...
  chunks：有效值为 `all`、`async`、`initial`，`initial` 是同步加载，`async` 是异步加载
...
这个分析不是基于我们的 bundle 文件，而是基于 sourcemap，需要生成 sourcemap
