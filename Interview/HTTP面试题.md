...
http3 选择了一个折衷的方法，基于 UDP 协议，在 UDP 的基础上实现多路复用
...
3. 客户端验证证书
    ...
    - 客户端使用 CA 的公钥对 Certificate Signature 进行解密，得到 Hash 值
    - 客户端使用与 CA 相同的 Hash 算法对证书信息进行计算，得到另一个 Hash 值
    - 对比两个 Hash 值，如果一致则说明证书未被篡改，验证通过

4. TLS 第三次握手

    客户端收到证书后，会生成一个**预主密钥（Pre-Master Key）**，并使用证书中的公钥对其进行加密，发送给服务端。这个预主密钥结合之前的 Client Random 和 Server Random，双方各自计算出相同的对称加密密钥（会话密钥）

5. TLS 第四次握手

    服务端收到加密的预主密钥后，用自己的私钥解密，然后结合 Client Random 和 Server Random 计算出会话密钥。之后服务端使用会话密钥加密一条"Finished"消息发送给客户端，客户端验证通过后也回复一条"Finished"消息，握手完成，后续通信都使用对称加密
...
1. 数据传输安全：HTTPS 使用 SSL/TLS 协议加密数据传输，保证数据传输的安全性和完整性
...
3. 缓存不利：HTTPS 的内容在某些情况下不能被缓存（如私有数据），因此需要经常请求数据，增加了带宽成本
...
3. 3xx 重定向 需要进行附加操作已完成请求
   ...
   - 302 Found 临时性重定向
   - 303 See Other 表示由于请求对应的资源存在于另一个URI,应用GET方法重新获取请求的资源
   - 304 Not Modified 表示客户端发送附带条件的请求时,服务器允许请求访问资源,但资源未发生变化的情况下
...
CDN 刷新（Refresh）与预热（Cache Pre-warming）
...
> 注意：Cookie/Session 示意图（原图链接：https://gitee.com/lilyn/pic/raw/master/lagoulearn-img/image-20230316172503583.png）
...
1. **可以窃取 cookie 信息**。恶意 JavaScript 可以通过 `document.cookie` 获取 cookie 信息，然后通过 XMLHttpRequest 或者 Fetch 加上 CORS 功能将数据发送给恶意服务器；恶意服务器拿到用户的 cookie 信息之后，就可以在其他电脑上模拟用户的登录，然后进行转账操作。
2. **可以监听用户行为**。恶意 JavaScript 可以使用 `addEventListener` 接口来监听键盘事件，比如可以获取用户输入的银行卡等信息，进而做很多违法的事情。
...
1. 输入网址 -- 告诉浏览器你要去哪里
2. 浏览器查找 DNS -- 网络世界是 IP 地址的世界，DNS 就是 IP 地址的别名。从本地 DNS 到最顶层 DNS 一步一步往上爬，直到命中需要访问的 IP 地址
