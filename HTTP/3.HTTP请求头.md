...
- `Accept`：我（客户端）需要什么样的 MIME 资源，比如 json 与 html，**不较常见，使用较少**
...
在浏览器中通过 Form 表单发送请求体会自动携带 `application/x-www-form-urlencoded` 请求头
...
同时 UA 请求头也可用作内容协商，`User-Agent` 还有以下实际应用场景
