...
- JavaScript字符串把每个 UTF1 单元当做一个字符来处理，所以处理非BMP(超出 U+0000 - U+FFFF 范围)的字符时，你应该格外小心
...
取值：-2^53  ~ 2^53            Number.MIN_VALUE  ~ Number.MAX_VALUE
...
// 可以看出 nextTick 是把任务放在当前宏任务后面，在微任务执行之前
// 通过 array.push 
process.nextTick(() => {
    console.log(6);
})
...
// hi()：这一步执行的是箭头函数，箭头函数继承上一个代码块的this，上一层是obj
hi();               //输出obj对象
let sayHi = obj.sayHi();
// sayHi()：隐式绑定丢失的情况
...
fun1();             //输出window
// obj.say()：执行的是箭头函数，当前 obj 中不存在 this，只能往上找，就找到了全局的 this
obj.say();          //输出window
```
...
// this 指向调用它的对象 b
var f = b.detail(); // 3
// b.detail 相当于普通的函数赋值给 c（方法变成函数），变量 c 才是真正的调用者
c(); // 1
...
// 函数 d 里直接执行形参 i，形参 i 是一个函数；相当于一个值被用在函数 d 里面，实际执行的是全局范围的 d => window
d(b.detail); // 1

// b.bibi() 实际上是把函数赋值给了变量 e，直接调用的是全局的 this => window
var e = b.bibi();
e(); // 1
```
