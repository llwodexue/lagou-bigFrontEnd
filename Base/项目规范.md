...
5. 组件名要带有业务属性，如 PriceList、 PriceDetail 等。禁止直接使用通用名，如：Form、List 等命名

6. vue 项目统一使用 scss，react 项目统一使用 less。可以在此基础上使用原子化 css

## 注释规范

1. TypeScript 接口、Hooks、Utils 的注释必须使用 [JSdoc 规范](https://jsdoc.app/)，使用 /** 内容 */ 格式，写明功能、参数、return 及 TODO。可用 VScode 插件生成标准格式
2. API 接口配置需要有详细的接口说明及 Yapi 文档地址
3. 所有组件、hook、store、等文件都必须添加创建者和创建日期。可用 VScode 插件生成标准格式
...
| **分支管理**         | **命名规范**                       | **解释**                                                     |
| -------------------- | ---------------------------------- | ------------------------------------------------------------ |
| main 主分支          | main                               | 稳定版本分支，上线完成回归后，由项目技术负责人从 release 分支合并进来，并打 tag |
