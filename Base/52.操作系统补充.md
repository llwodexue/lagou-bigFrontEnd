...
- 当拷贝完毕后 DMA 会发出数据读完信号之后通知 CPU，收到通知后会把内核缓冲区的数据拷贝到用户缓冲区
- 当内核态切换为用户态后会唤醒以及阻塞的用户进程，用户进程就会向 CPU 发起 write() 系统调用，通过 CPU 时间片轮询，用户进程向 CPU 发起 write() 系统调用，将数据通过 TCP/IP 协议栈发送到网卡设备
...
MMAP+write 将 PageCache 数据映射到用户缓存区
