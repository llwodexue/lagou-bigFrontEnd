...
```js
// 注意：args.reverse() 会改变原始数组，导致重复调用 compose 时出现问题
// 应使用 [...args].reduceRight((acc, fn) => fn(acc), value) 替代
...
function deepCloneV2(originObj, map = new Map()) {
  if (isObject(originObj)) {
    ...
    // 注意：WeakMap/WeakSet 构造函数不接受参数，传入会抛出 TypeError
    // 注意：Map/Set 的上述构造方式仅为浅拷贝，嵌套对象仍共享引用
...
// 注意：当前实现未处理 BigInt、Error、ArrayBuffer 等特殊类型
```
...
// 方法2：使用 Map 去重
Array.prototype.uniqByMap = function () {
  const map = new Map()
  let arr = []
...
// 方法3：排序后去重
Array.prototype.uniqBySort = function () {
  this.sort((a, b) => a - b)
  let j = 0
