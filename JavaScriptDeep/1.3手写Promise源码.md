...
**注意：**  举个例子 `var obj = { n: 10, x: obj.n *10 }`，因为 obj 还没有创建完，而在创建属性 x 时是获取不到 obj.n 的。因为全局作用域只声明了 obj，却没有赋值（`obj -> undefiend`），promise 也是有这样的情况的，我们可以使用异步任务，让其赋值完成
...
- 如果参数是普通值，就直接将值转换为成功的 Promise 实例
