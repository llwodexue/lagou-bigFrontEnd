...
4. 修改本地 config
   ...
   这里需要注意，需要把 `~/.ssh/id_rsa.pub` 文件粘贴到远程服务器 `~/.ssh/authorized_keys` 中，这里可以使用 `ssh-copy-id` 工具来代替 `<ctrl + c>、<ctrl + v>` 操作 
...
2. `ssh -v -E ssh_lyn_root.log lyn` 通过 -E 参数可将调试日志保存至某个文件中
...
> [使用Chrome浏览器访问6000端口提示 ERR_UNSAFE_PORT](https://blog.csdn.net/qq_28817739/article/details/84501454)
>
