...
2. 功能型（和数据库打交道）：中后台页面的快速落地、列表/表单的快速搭建
3. 互动型：游戏引擎
...
1. **底层**：服务依赖，可以按活动的四个生命周期分为四大类：活动配置中：（规则引擎：比如周一只能用 KFC 的汉堡优惠券，周二只能用可乐优惠券，规则引擎对于低代码来说是比较重要的一环）投放时：**依托低代码为各个投放系统快速搭建广告投放页面；**运行时：和用户行为相关，比如行为系统、任务系统；活动结束后：（我们也有归因平台、数据统计平台做一些数据分析）。
2. **第二层：**是前端的一些底层能力
...
3. 团队协作问题： 最开始活动平台用于小部门，后来大家觉得还不错各个业务部门都开始使用了起来。所以各个业务团队之间就面临着共建、协作的问题。
...
理想情况是基于组件的粒度进行加载，现实是 Qiankun 只提供了页面基本的引用。如果就 Qiankun 来进行应用升级，这对于微组件来说会过重。**如果引进来就有一点杀鸡焉用牛刀的味道了**
...
那基于这个思路，我需要从 **数据库** 拿到所有组件信息，去生成 webpack.config 的文件、得到依赖关系）。如果组件越来越多，这份配置文件也将会变得无比巨大。本质是没有解决枚举这个问题的，只是将业务侧枚举交给 webpack 枚举而已
...
**ESbuild** 新一代的构建工具嘛，相信大家也是耳熟能详了。Esbuild 在资源打包这种 CPU 密集场景下，基于 Go 更具性能优势
...
1. stream-import 可以类比为 snowpack 的 streaming import，可以实现从远程加载 pkg 包，并且转换成 ArrayBuffer 的形式给到 ESbuild。官网给到的 http 示例是基于 Node 环境的，替换成浏览器环境即可
...
基于esbuild的 wasm 能力，可以打包 Web 化，利用 virtual module 的能力可以完成http的请求加载，以及内网包的复用
