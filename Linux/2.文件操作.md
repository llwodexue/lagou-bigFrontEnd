...
`IDLE` 表示当前用户已经处于不活跃多长时间，`.` 表示当前仍在活跃状态
...
1. 我们修改了文件的 mode，在 git 中是否有更改操作
   ...
   使用 chmod 更改文件的 mode，git 中是没有更改操作的
...
- 其中 `<<EOF`，称作 `Here Document`，当最终写入 EOF（End Of Line）时，则 heredoc 会停止输入
...
**glob：** glob pattern 的缩写，使用通配符来匹配大量文件。比如：`rm * .js` 就可以删除当前目录所有 js 文件
