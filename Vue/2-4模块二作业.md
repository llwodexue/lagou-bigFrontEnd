...
- Observer 类中判断该 value 是数组还是对象，进行不同的处理
  - 数组的响应化处理，是重写 push、pop、sort 等会修改原数组的方法，内部会先调用原始数组方法修改数据，再调用 `dep.notify()` 触发更新，然后遍历数组中的成员，判断其类型决定是否调用 observe 方法
...
- 模版编译入口函数是 `compileToFunctions`，首先判断是否有缓存好的 render 函数，如果没有，则调用 compile。compile 负责将 template 和 options 传递给 `baseCompile`（合并 options 是由 `mergeOptions` 负责的）
