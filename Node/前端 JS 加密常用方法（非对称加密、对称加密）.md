...
彩虹表是对于散列函数做逆运算的表（空间换时间），密码安全度比较低时，可以用彩虹表碰撞去破解的，可以在 [Free Rainbow Tables](https://freerainbowtables.com/) 上下载进行试验，如果试图解密，可以使用 [CMD5](https://cmd5.com/) 进行解密
...
const hmacHash = (type, str) => crypto.createHmac(type, salt).update(str).digest('hex')

const psw = '123'
const md5 = str => hmacHash('md5', str)
...
CryptoJS 加密出的结果是一个 `CipherMessage` 对象（包含 ciphertext、iv、salt 等属性的封装对象），所以需要调用 `.toString()` 或 `+ ''` 将其转换为 BASE64 字符串
...
> 注意：BASE64 是一种编码方式，不是加密算法。它只是将二进制数据转换为可打印的 ASCII 字符，任何人都可以解码还原，不具备保密性。
...
非对称加密会产生一对密钥（公钥负责加密、私钥负责解密）。抗抵赖性通过数字签名实现：用私钥签名、公钥验证。因为只有签名者持有私钥，所以他人无法伪造签名，签名者也无法否认自己签署过的内容。常见算法 RSA（大质数 ）、Elgamal、背包算法、Rabin、D-H、ECC（椭圆曲线加密算法）
